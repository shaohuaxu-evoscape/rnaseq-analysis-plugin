...
Usage:
    python rnaseq_gsea.py -c configs/analysis_case.yaml
    python rnaseq_gsea.py -c configs/analysis_case.yaml --dry-run
"""
import argparse
import os
import sys
...
from scripts.steps.gsea import run


def main():
    parser = argparse.ArgumentParser(description="GSEA (step 3d)")
    ...
    if args.dry_run:
        print("[DRY RUN] Would run step 3d: GSEA")
        return
