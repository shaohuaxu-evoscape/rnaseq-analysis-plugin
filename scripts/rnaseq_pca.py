...
Usage:
    python rnaseq_pca.py -c configs/analysis_case.yaml
    python rnaseq_pca.py -c configs/analysis_case.yaml --dry-run
"""
import argparse
import os
import sys
...
from scripts.steps.pca import run


def main():
    parser = argparse.ArgumentParser(description="PCA (step 2c)")
    ...
    if args.dry_run:
        print("[DRY RUN] Would run step 2c: PCA")
        return
