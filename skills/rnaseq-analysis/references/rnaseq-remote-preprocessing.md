...
Sample information is read from `{work_dir}/sample_manifest.tsv` (uploaded from local `inputs/sample_manifest.tsv` by the wizard). The manifest has columns: `sample_id  condition  timepoint  r1_file  r2_file`.
...
MANIFEST="{work_dir}/sample_manifest.tsv"
